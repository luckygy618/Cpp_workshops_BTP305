...
{
	int index = 0;//a counter
	std::vector<int> holder;
	//find all the items need to be filled, including repeated item name like GPU and Monitor
	for (size_t i = 0; i < m_cntItem; i++) {
		if (m_lstItem[i]->m_itemName == station.getItemName()) {
			holder.push_back(i);//save the index of item in m_lstItem
			index++;
		}
	}

	//loop the holder to fill all the items
	if (index > 0) {
		std::for_each(holder.begin(), holder.end(), [&](int i) {
