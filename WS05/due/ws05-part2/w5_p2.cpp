...
int main(int argc, char** argv)
{
	...
	sdds::Collection<sdds::Book> library("Bestsellers");
	{
		...
		while (!load.eof() && skip < 4) { //read from file and save the first 4 books
			bool comment = false;
			std::string temp = "";
			getline(load, temp);
			if (temp.substr(0, 1) == "#") {//skip the # lines
				comment = true;
			}
	...
	//a lambda function that catches all the & reference in the range and changes them
	auto fun = [&](Book& book) {
	...
	for (size_t i = 0u; i < library.size(); i++) {
		fun(library[i]);//use the lambda to changes the prices
	}
	...
	Movie movies[5];
	{
		...
		while (!loadmovie.eof() && stored < 5) {
			...
			if (comment == false) {
				movies[stored] = Movie(temp); //read from file and set up movies
				stored++;
			}
			temp = "";
		}
	...
	catch (std::exception& error) { //catches the standard exception
		std::cout << "EXCEPTION: " << error.what() << std::endl;//display the exception
	}
	std::cout << "-----------------------------------------\n\n";
	std::cout << "-----------------------------------------\n";
