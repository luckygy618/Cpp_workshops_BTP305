...
void loadData(const char* filename, sdds::Autoshop& as)
{
	...
	while (file)
	{
		...
		//           "Invalid record!<endl>"
		try {
			sdds::Vehicle* aVehicle = sdds::createInstance(file); //createInstance will throw const char* "Unrecognized record type:" + [type]
																//in createInstance(), it will call constructor and throw string "Invalid record!"
			if (aVehicle)
				as += aVehicle;
		}
		catch (std::exception& err) { //catch standard exception
			std::cout << err.what() << std::endl;
		}
		catch (std::string err) { // catch string "Invalid record!"
			std::cout << err << std::endl;
		}
		catch (const char* err) {//catch const char* "Unrecognized record type:" + [type]
			std::cout << err<< std::endl;
		}
	}
}
...
int main(int argc, char** argv)
{
	...
	std::list<const sdds::Vehicle*> vehicles;
	{
		...
		auto fastVehicles = [](const sdds::Vehicle* vehicle) { //pass a parameter const sdds::Vehicle* and return a bool
			if (vehicle->topSpeed() > 300) { return true; }
			else { return false; }
		};
	...
	std::cout << std::endl;
	{
		...
		auto brokenVehicles = [](const sdds::Vehicle* vehicle) {//pass a parameter const sdds::Vehicle* and return a bool
